...
cityName("Berlin"); // The city name does not begin with los or new
...
function sumCompute(integersArr){
...
sumCompute([5, 10, 15, 45, 5]); // 80

//___________________________________

function sumCompute2(a, b, c, d) {
    let sumCompute = a + b + c + d;
    console.log(sumCompute);
}

sumCompute2(5, 10, 20, 30); // 65
sumCompute2(10, 20, 30, 40); // 100
...
function fullName(first, last) {
    let fullName = `${first.trim()}, ${last.trim()}`; 
    console.log(fullName);  
}

fullName("Mary", "Green"); // Mary, Green
...
integer(345); // false - integer is not divisible by 100
integer(1000); // true - integer is divisible by 100
...
adjEnding(["smart", "clever", "strong", "tall", "fast"]); 
// [ 'smarter', 'cleverer', 'stronger', 'taller', 'faster' ]

//6. Write a program that checks whether a number is even or odd and returns "even" for even 
...
//Write a program that classifies the missing angle as either "acute", "right", or "obtuse" based 
//on its degrees.
//
...
//For example: 11, 20 should return "obtuse", since the missing angle would be 149 degrees, which 
//makes it obtuse.
//
//Examples
...
function angleThree(angleThree) {

    function missingAngle(angleOne, angleTwo) {
        let sumOfAngles = angleOne + angleTwo;
        let missingAngle = 180 - sumOfAngles;
        console.log(missingAngle);
    }
    missingAngle(67, 35);
...
wordCount("This can be however long we want it to be, it doesn't matter!");
